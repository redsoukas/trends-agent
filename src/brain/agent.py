...
class ContentAgent:
    ...
    def analyze_metadata_trends(self, videos: List[Dict]) -> Optional[Dict]:
        ...
        try:
            ...
            prompt = f"""Analyze YouTube trending patterns using metadata:
            
            OVERVIEW:
            • Total Videos: {len(videos)}
            • Total Views: {total_views:,}
            • Categories: {', '.join([cat for cat, _ in category_counts[:5]])}
            
            VIDEO TITLES:
            {chr(10).join(f'• {title}' for title in titles[:10])}
            
            TOP CHANNELS:
            {chr(10).join(f'• {channel} ({count} videos)' for channel, count in channel_counts[:5])}
            
            Analyze content trends, creator patterns, and market opportunities.
            Provide JSON format insights."""
            ...
            if response:
                response['metadata_stats'] = {
                    ...
                    'top_categories': [cat for cat, _ in category_counts[:3]],
